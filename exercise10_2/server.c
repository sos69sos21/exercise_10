...
int main(int argc, char *argv[])
{
    ...
    // bind the address information to the listener socket with error checking
    if (bind(listener, (struct sockaddr *)&serverAddr, sizeof(serverAddr)) < 0)
    {
        perror("Error binding socket address information");
        fprintf(stderr, "bind error: %s\n", strerror(errno));
        close(listener);  // Close the listener socket to clean up
        return 1;
    }
    ...
    while (1)
    {
		...
		while(1){
			...
			if ((bytesRead = recv(connectedSocket, data, sizeof(data), 0)) > 0)
			//if ((bytesRead = recv(connectedSocket, data, sizeof(data), 0) > 0))
			{
			...
			{
				perror("Recv function returned an error\n");
			}
		}
